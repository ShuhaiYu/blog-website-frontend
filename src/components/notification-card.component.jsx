...
import NotificationCommentFeild from "./notification-comment-field.component"
    ...
    return (
        <div className={"p-6 border-b border-grey border-l-black " + (!seen ? " border-l-2" : "")}>
...

                {
                    type !== "like" ?
                        <>
                            ...
                            <button className="underline hover:text-black" onClick={(e) => handleDeleteClick(comment._id, "comment", e.target)}>Delete</button>
                        </> : null
                }
...

            {
                isReplying ?
                    <div className="mt-8">
                        <NotificationCommentFeild _id={_id} blog_author={user} index={index} replyingTo={comment._id} setReplying={setIsReplying} notification_id={notification_id} notificationData={notificationState} />
                    </div> : null
            }

            {
                reply ?
                    <div className="ml-20 p-5 mt-5 rounded-md bg-grey">
                        ...
                        <button className="underline hover:text-black ml-14 mt-2" onClick={(e) => handleDeleteClick(comment._id, "reply", e.target)}>Delete</button>
                    </div> : null
            }
